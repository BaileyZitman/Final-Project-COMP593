...
#  "FinalProject.py" to complete the objective and is called by the RunAPODPBMZ.ps1 script. 
#  
#  For a detailed description on the functionality of this script vist the ReadMe file
#
# Usage
...
if (Test-Path -Path "./PBMZ-db-FP.sqlite"){ #test to make sure database exists, if it does continue
    ...
    if ($connection_results){ #if connection returns a hash continue else no database/entry for current date
        Write-Host "The database was located sucessfully and an entry already esists for this day" -ForegroundColor "Green"
        Write-Host "The Hash of that image is being compared with the files in: " -ForegroundColor "Green" -NoNewline ; Write-Host $directory_to_search -ForegroundColor "Yellow" -NoNewline
        Write-Host " and " -NoNewline -ForegroundColor "Green" ; Write-Host ".\Saved Images" -ForegroundColor "Yellow"
